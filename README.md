...
- Параметры для запуска прописаны в config.go (http, db параметры). Параметры подставляются из `.env` (который можно создать из `example.env`) либо в самой структуре прописаны дефолтные рабочие параметры.
- В качестве БД используется Postgres, ее можно поднять через Docker
- Миграции для БД находятся в `migrations/table.sql`
- Команды для поднятия Docker и запуска приложения прописаны в `Makefile`