...
- Описание
    ...
    Номер следующей подсказки можно взять из запроса `/team/contest/tasks`. Если запросить подсказку с бОльшим номером, то вернется ошибка `hint_num_not_exist`. Если с меньшим, то вернется последняя показанная подсказка.
